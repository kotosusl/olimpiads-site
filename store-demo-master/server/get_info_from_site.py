...
"""def list_of_olimpiads(olimp, params):
    ...
    lst = []
    for one_sub_code in subs:
        new_params = params
        new_params[f'subject[{subs[one_sub_code]}]'] = 'on'
        new_params['cnow'] = '0'
        res = requests.get(url, params=new_params)
        soup = BeautifulSoup(res.text, 'html.parser')
        ...
        while res:
                ...
                if subjects is None:
                    ...
                    if classes:
                        ...
                        if len(classes.split('-')) < 2 and len(classes.split('–')) < 2 and ',' not in classes:
                            ...
                            min_class = int(classes.split('-')[0])
                            max_class = int(classes.split('-')[1])
                    subjects = (subjects.find_all('span', class_='subject_tag') if subjects.find_all('span', class_='subject_tag') else subjects.find_all('span', class_='subject_tags'))
            ...
            new_params['cnow'] = str(int(new_params['cnow']) + 20)
    return lst"""
